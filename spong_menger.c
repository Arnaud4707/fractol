...
int inMenger2D(double x, double y, int maxIter) {
    for (int i = 0; i < maxIter; i++) {
        ...
        // trou si au centre ET sur un axe
        if (xi == 1 && yi == 1) {
            return 0; // pixel vide
        }
...
void	drawMenger2D(t_vars *vars, int depth)
{
    for (int y = 0; y < vars->hauteur; y++) {
        for (int x = 0; x < vars->largeur; x++) {
            double nx = (double)x / vars->largeur;   // normalisé 0..1
            double ny = (double)y / vars->hauteur;

            nx *= pow(3, depth); // ajuster au niveau fractal
            ny *= pow(3, depth);

            if (inMenger2D(nx, ny, depth)) {
                my_mlx_pixel_put(vars->img, x, y, 0xFFFFFF); // blanc
            } else {
                my_mlx_pixel_put(vars->img, x, y, 0x000000); // noir
            }
        }
    }
}
