...
int	loop_hook(void *arg)
{
    ...
    // appelle la génération multi-thread qui ajoute SAMPLES/THREADS par thread
    buddhabrot_colored_thread_animation(vars);
	printf("zoom : %f | shift : %f\n", vars->zoomm, vars->hue_shift);
    // affiche l'image
...
void	buddhabrot_colored_thread_animation(t_vars *vars)
{
	...
	// }
	// static int cpt;
	// if (vars->hue_shift < 0.000001 && vars->hue_shift > -0.000001)
	// 	vars->hue_shift = rand()%10;    // rotation de palette
	// if (cpt == 10)
	// {
    // 	vars->hue_shift *= 1.01;    // rotation de palette
    // 	cpt = 0;
	// }
	// else
	// 	cpt++;
